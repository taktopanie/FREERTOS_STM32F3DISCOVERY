...
#define OS_FSYS 			48000000L   // MCU core frequency of Flasher ARM Pro V4
...
#define OFF_CC              0x20        // Status clear register
#define OFF_DR_RX           0x24        // Data RX register
#define OFF_DR_TX           0x28        // Data TX register
...
#define GPIO_UART_TX_BIT    4               // USART1 TX: Pin pc4
#define GPIO_UART_RX_BIT    5                 // USART1 RX: Pin pc5
...
#define USART_CC            *(volatile uint32_t*)(USART_BASE_ADDR + OFF_CC)
#define USART_DR_RX         *(volatile uint32_t*)(USART_BASE_ADDR + OFF_DR_RX)
#define USART_DR_TX         *(volatile uint32_t*)(USART_BASE_ADDR + OFF_DR_TX)
...
void USART1_IRQHandler(void) {
  ...
  if (UsartStatus & (1 << USART_TXE)) {                // Tx (data register) empty? => Send next character Note: Shift register may still hold a character that has not been sent yet.
    ...
    if (r == 0) {                          // No more characters to send ?
      ...
      //USART_CC = 0xFFFFFFFF;
      USART_DR_TX = v;  // Start transmission by writing to data register
    }
  }
}
...
void HIF_UART_Init(uint32_t Baudrate, UART_ON_TX_FUNC_P cbOnTx, UART_ON_RX_FUNC_P cbOnRx) {
  ...
  RCC_AHB1ENR |= (1 <<  19);        // Enable IO port C clock
